...
public class MovementSystemB : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        MovementVelocity = Input.GetAxis("Horizontal");
    ...
    private void FixedUpdate()
    {
        ...
        gameObject.transform.Translate(new Vector3(MovementVelocity * speed * Time.fixedDeltaTime, 0, 0));
    ...
    private void RaycastGround()
    {
        ...
        Debug.DrawRay(new Vector2(transform.position.x, transform.position.y-1.3f), new Vector2(0,-0.1f));
        Debug.DrawRay(new Vector2(transform.position.x + 0.47f, transform.position.y - 1.3f), new Vector2(0, -0.1f));
        Debug.DrawRay(new Vector2(transform.position.x - 0.47f, transform.position.y - 1.3f), new Vector2(0, -0.1f));
        ...
        for (int i = 0; i < raycastHit2D.Length; i++)
        {
            if (StayOnGround == false)
            {
                ...
                {
                    if (raycastHit2D[i].collider.tag == "Ground")
                    {
                        if (isSit == false)
                        {
                            Stay.enabled = true;
    ...
    private void RaycastWalls()
    {
        ...
        Debug.Log(raycastHitRightWall[0].collider.name);
        ...
        {
            isLeft = true;
            if (raycastHitLeftWall[0].collider.tag == "Wall") { rb.velocity = new Vector2(0, -0.2f); isSlide = true; isNearWall = true; }
        }
        else
